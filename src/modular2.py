...
if model and predictors: # Only proceed if model and predictors are loaded
    ...
    if st.sidebar.button("Predict Cluster"):
        ...
        try:
            ...
            if scaler:
                 # Ensure the input DataFrame has the same columns in the same order as the scaler expects
                 # This might require reindexing if the 'predictors' list order differs from scaler's features
                 # For simplicity, assume 'predictors' list matches scaler's feature order
                 scaled_input = scaler.transform(input_df)
                 input_features = scaled_input
                 st.write("Data scaled successfully.") # Optional: confirmation
            else:
                 # If no scaler was loaded, use the raw input (or handle as appropriate)
                 input_features = input_df.values
                 st.warning("Proceeding without scaling. Ensure input values are appropriately scaled if required.")
            ...
            # Add more elif conditions for other clusters found in the notebook analysis
                 ...
                 st.write(f"Interpretation for Cluster {predicted_cluster} needs to be defined based on notebook analysis.")
...
# Visualizar (si scaled_features_train se cargó correctamente)
#st.title('Predicción de Clúster de Cliente')

# Campos de entrada del usuario (ejemplo)
#recency = st.number_input('Recency', value=10)
#frequency = st.number_input('Frequency', value=5)
#monetary = st.number_input('Monetary', value=100)

#if st.button("Predict Cluster"):
    #new_customer_data = {'Recency': recency, 'Frequency': frequency, 'Monetary': monetary}  # ... otros datos del cliente
    #predicted_cluster, scaled_new_customer_data = predict_cluster(
        #new_customer_data, model, scaler, predictors
    #)

    #st.write(f'El cliente pertenece al Clúster: {predicted_cluster}')

    # Visualizar
    #if scaled_features_train is not None:
        #plot_pair_plots_with_new_customer(
           # scaled_features_train, model.labels_, scaled_new_customer_data, predictors, predicted_cluster
        #)
    #else:
        #st.warning('No se pudieron cargar los datos de entrenamiento escalados. No se mostrará la visualización.')
